...
const operationWindowJobs = [
  ...["monday", "tuesday", "wednesday", "thursday", "friday"].map((day) => postJob({
    ...
    schedule: { type: "weekly", days: [day], time: String(process.env.MAST_AM_OPERATION_TIME || "07:45"), timezone: LOCAL_TIME_ZONE },
    hookEnv: null,
  ...
  ...["monday", "tuesday", "wednesday","thursday"].map((day) => postJob({
    ...
    schedule: { type: "weekly", days: [day], time: String(process.env.MAST_PM_OPERATION_TIME || "15:45"), timezone: LOCAL_TIME_ZONE },
    hookEnv: null,
    fallbackUrl: `${aimsBaseUrl()}/ops/run/${day}-pm`,
    targetUrl: `${aimsBaseUrl()}/ops/run/${day}-pm`,
    targetPath: `/ops/run/${day}-pm`,
    authEnv: "AIMS_API_KEY",
  })),
  postJob({
    id: "operation-friday-pm",
    group: "operations",
    description: "Friday extended AIMS operating window: Blotato evening short, podcast pipeline, and Saturday/Sunday Zernio scheduling before weekend standby.",
